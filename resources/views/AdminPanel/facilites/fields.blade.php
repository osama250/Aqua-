...
<div class="card-body border-top p-9">
    ...
    <div class="tab-content mt-5" id="myTabContent">
                        ...
                        {{-- <div class="col-lg-12 fv-row fv-plugins-icon-container">
                            @foreach ($cabins as $cabin)
                                <div class="form-check">
                                    <input type="checkbox" name="cabin_id[]" value="{{ $cabin->id }}" class="form-check-input" id="cabin-{{ $cabin->id }}">
                                    <label class="form-check-label" for="cabin-{{ $cabin->id }}">{{ $cabin->title }}</label>
                                </div>
                            @endforeach
                        </div> --}}
                        <div class="col-lg-12 fv-row fv-plugins-icon-container">
                            @foreach ($cabins as $cabin)
                                <div class="form-check">
                                    <input type="checkbox" name="cabin_id[]" value="{{ $cabin->id }}" class="form-check-input" id="cabin-{{ $cabin->id }}"
                                        @if( isset($facilite) && $facilite->cabins->contains($cabin->id) )
                                            checked
                                        @endif
                                    >
                                    <label class="form-check-label" for="cabin-{{ $cabin->id }}">{{ $cabin->title }}</label>
                    ...
                    <div class="row">
                            <div class="col-lg-12 fv-row fv-plugins-icon-container">
                                <select name="category_id"
                                        class="form-control form-control-lg form-control-solid mb-3 mb-lg-0 ">
                                    <option value="Choosse"> Choosse Category </option>
                                    @foreach ( $categories as $cat )
                                        <option value="{{ $cat->id }}" {{  isset( $facilite ) && $cat->id == $facilite->category_id ? 'selected' : '' }}>
                                            {{ $cat->title }}
                                        </option>
        ...
        <div class="row mb-3">
            <!--begin::Label-->
            <label class="col-lg-4 col-form-label required fw-semibold fs-6">{{ __('lang.order') }}</label>
            <!--end::Label-->
                ...
                <div class="row">
                    ...
                    <div class="col-lg-12 fv-row fv-plugins-icon-container">
                        <input type='number' name="order"
                            class="form-control form-control-lg form-control-solid mb-3 mb-lg-0 "
                            placeholder="{{ __('lang.order') }}" value="{{isset($facilite) ? $facilite->order : '' }}">
                        <div class="fv-plugins-message-container fv-plugins-message-container--enabled invalid-feedback">
                        </div>
                    </div>
                    <!--end::Col-->
  ...
  <div class="fv-row mb-8">
    ...
    <div class="dz-clickable dropzone" id="kt_modal_create_project_settings_logo">
        ...
        <div class="dz-message needsclick">
            ...
            @if( isset($facilite) && count($facilite->photos) > 0 )
                @foreach ($facilite->photos as  $photo)
                    <div class=" dz-processing  dz-success dz-complete">
                        <div class="" style="width: 400px; height: 200px;" >
                            <img data-dz-thumbnail="" alt="{{$facilite->title}}"
    ...
    <script>
        ...
        var myDropzone = new Dropzone("#kt_modal_create_project_settings_logo", {
            url: '{{url(LaravelLocalization::getCurrentLocale()."/image/upload/facilites")}}',
            paramName: "photo",
            //maxFilesize: 2,
